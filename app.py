...
def render_dashboard(
    ...
    if os.path.exists(DUMMY_TANK_PATH):
        ...
        if not dummy_tank_missing_columns:
            dummy_tank_filtered = df_dummy_tank[required_dummy_cols + (["Zone"] if "Zone" in df_dummy_tank.columns else [])].copy()
...
def render_dummy_tank_details(
    ...
    display_cols = ["Material", "Plant", "Storage Location", "Base Unit of Measure", "Unrestricted"]
    ...
    if "Unrestricted" in display_df.columns:
        styled_df = styled_df.format({"Unrestricted": "{:,.0f}"})
    ...
    with col1:
        ...
        if not top5_plants.empty:
            st.dataframe(top5_plants, use_container_width=True, hide_index=True)
        else:
    ...
    with col2:
        ...
        if not top5_zones.empty:
            st.dataframe(top5_zones, use_container_width=True, hide_index=True)
        else:
